...
fn setEcho(enable: bool) !void {
    switch (builtin.os.tag) {
        .windows => {
            ...
            const mode = std.os.windows.CONSOLE.USER_IO.GET_MODE.Data;

            const newMode = if (enable) mode | ENABLE_ECHO_MODE else mode & ~ENABLE_ECHO_MODE;

            _ = std.os.windows.CONSOLE.USER_IO.SET_MODE(newMode);
        },
        else => {
...
fn getPassword(io: std.Io, allocator: std.mem.Allocator, reprompt: bool) ![]u8 {
    try setEcho(false);
    defer setEcho(true) catch @panic("Failed to re-enable terminal echo");
    ...
    const p1out = try allocator.alloc(u8, p1.len);
    @memcpy(p1out, p1);
...
const Cli = struct {
    ...
    fn parse(args: []const [:0]const u8) Self {
        var out: Self = .{};

        var nextIsInput = false;
        var nextIsOutput = false;
        for (args) |arg| {
            var positionalTriggered = false;
            if (nextIsInput) {
                out.input = arg;
                nextIsInput = false;
                positionalTriggered = true;
            }
            if (nextIsOutput) {
                out.output = arg;
                nextIsOutput = false;
...
fn encrypt(io: std.Io, allocator: std.mem.Allocator, input: []const u8, output: ?[]const u8) !void {
    ...
    var outputFile = if (output) |path| try std.Io.Dir.cwd().createFile(io, path, .{}) else blk: {
        ...
        const name = try std.fmt.allocPrint(allocator, "{s}.enc", .{fileName});
    ...
    const progress = std.Progress.start(io, .{ .root_name = "Encrypting file...", .estimated_total_items = @as(usize, total) });
    defer progress.end();
...
fn decrypt(io: std.Io, allocator: std.mem.Allocator, input: []const u8, output: ?[]const u8) !void {
    ...
    var outputFile = if (output) |path| try std.Io.Dir.cwd().createFile(io, path, .{}) else blk: {
        ...
        const name = iter.next() orelse return error.InvalidFilename;
    ...
    const progress = std.Progress.start(io, .{ .root_name = "Decrypting file...", .estimated_total_items = @as(usize, total) });
    defer progress.end();
